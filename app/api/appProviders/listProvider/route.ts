...
export async function GET(req: Request) {
  ...
  try {
    ...
    const [providers]: [any[], any] = await connection.query('SELECT id as provider_id, name as provider_name, logo as provider_logo FROM provider');
    ...
    const providersData = await Promise.all(
      providers.map(async (provider) => {
        ...
        const [processes]: [any[], any] = await connection.query(
          'SELECT id as process_id, name as process_name FROM process WHERE provider_id = ?',
